...
@st.cache_resource
def load_vector_db():
    # --- AUTO-INGEST FALLBACK FOR STREAMLIT CLOUD ---
    if not os.path.exists(DB_DIR) or not os.listdir(DB_DIR):
        st.info("⚡ Chroma DB missing on server. Running automatic ingestion from polity_docs/...")
        try:
            import ingest_docs
            ingest_docs.main()
            st.success("✅ Vector Database indexed successfully!")
        except Exception as e:
            st.error(f"Auto-ingestion warning: {e}")
    # -----------------------------------------------
