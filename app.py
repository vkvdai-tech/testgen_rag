...
st.set_page_config(page_title="UPSC Polity MCQ Generator (V45 Direct)", page_icon="📚", layout="wide")
...
with st.sidebar:
    ...
    selected_provider = st.selectbox(
        ...
        index=0,
        help="Choose which LLM provider runs the pipeline."
    )
...
def run_r1_retriever(micro_topic, requested_qty, custom_instr):
    """STAGE 1: RETRIEVAL AGENT - Constructs Structured Evidence Package"""
    db_chunks = []
    try:
...
Prior Generated Questions Context: {json.dumps(st.session_state.generated_questions_history)}
...
def run_v45_generator(evidence_package, qty):
    """STAGE 2: V45 GENERATION AGENT - Generates Questions across 18 UPSC Formats without duplicates"""
...
if st.button("🚀 Generate Question Bank", type="primary"):
    ...
    for b in range(batches_count):
        current_batch_qty = min(batch_size, selected_count - len(questions_master))
        if current_batch_qty <= 0:
            break

        # Stage 1: Retrieval
        status_text.text(f"Batch {b+1}/{batches_count}: Running Stage 1 (Retrieval Agent)...")
        try:
        ...
        # Stage 2: Direct Generation with 18-Formats & Deduplication
        status_text.text(f"Batch {b+1}/{batches_count}: Running Stage 2 (V45 18-Format Engine)...")
        try:
            batch_questions = run_v45_generator(evidence_pkg, current_batch_qty)
            
            for q in batch_questions:
                questions_master.append(q)
                # Store full concept summary to prevent repeating concepts in future batches
                concept = q.get("tested_concept_summary", q.get("question_text", "")[:120])
    ...
    status_text.success(f"Pipeline Execution Complete! {len(questions_master)} Unique Questions Generated.")
    
    # Format Questions into Final Output Paper
    formatted_paper_blocks = []
    for idx, q in enumerate(questions_master, 1):
